...
let data = null;
                    ...
                    html += `
                        <div class="tree-item" style="margin-left: 48px;">
                            <div class="tree-node">
                                ...
                                ${skill.is_duplicate ? '<span class="badge" style="background: #ef4444;">重複</span>' : ''}
                            </div>
                        </div>
                    `;
                });
                    ...
                    html += `
                        <div class="tree-item tree-indent">
                            <div class="tree-node">
                                ...
                                <span>${skill.name}</span>
                                ${skill.dirty ? '<span class="badge" style="background: #f59e0b;">未提交</span>' : ''}
                            </div>
                        </div>
                    `;
                });
        ...
        function renderCommandsTree() {
            ...
            data.categories.agents.items
                ...
                .forEach(agent => {
                    // 從 agent 路徑提取專案名稱（例如從 "DEV/.claude/agents/team001-xxx.md" 提取 "team001"）
                    const pathParts = agent.path.split('/');
                    const fileName = agent.name;

                    // 檢查是否有對應的 skill（同名或相同前綴）
                    ...
                    // 額外檢查：team001, dopeman, dev-team-* 等團隊型 skills
            ...
            sortedCommands.forEach(cmd => {
                ...
                html += `<div class="tree-item">
                    <div class="tree-node">
                        ${isCoordinator ? '<span class="badge coordinator" style="margin-right: 8px;">協調者</span>' : ''}
        ...
        function renderDevProjectsTree() {
            ...
            if (projects.length === 0) {
                tree.innerHTML = '<div style="text-align: center; color: #999; padding: 40px;">尚無開發專案</div>';
                return;
            }
            ...
            projects.forEach(project => {
                ...
                const techStackHtml = project.tech_stack.length > 0
                    ...
                    : '<span style="color: #999; font-size: 0.85em;">未偵測到技術棧</span>';

                // Git 狀態
                const gitStatus = project.is_dirty
                    ? '<span style="color: #f59e0b;">● 未提交</span>'
                    : '<span style="color: #10b981;">✓ 乾淨</span>';

                // Claude Team 狀態
                const claudeTeam = project.has_claude_team
                    ? '<span style="color: #667eea;">✓ Claude Team</span>'
                ...
                html += `
                    <div class="project-card">
                        ...
                        <div class="tech-stack">
                            <span style="font-weight: 600; color: #666; font-size: 0.85em;">技術棧:</span>
                            ${techStackHtml}
                        </div>
        ...
        async function saveSettings() {
            try {
                ...
                // 使用 Python 腳本儲存（因為瀏覽器無法直接寫檔案）
                // 實際上這裡需要一個 API endpoint 來處理儲存
                // 暫時只更新本地資料和顯示成功訊息
                console.log('儲存設定:', preferencesData);

                // TODO: 實作實際的儲存功能（需要後端 API）
                // 可能的解決方案：
                // 1. 使用 Python HTTP server 提供 POST endpoint
                // 2. 使用 localStorage 暫存（但需要重新掃描時寫回檔案）
                ...
                alert('設定已更新！\\n\\n注意：目前設定僅在本次 Dashboard 會話中生效。\\n若要永久保存，請修改：\\n~/.claude/memory/dopeman/user-preferences.json');
            } catch (error) {
                console.error('儲存設定失敗:', error);
                alert('儲存設定失敗！請檢查控制台錯誤訊息。');
            }
        }
        ...
        function renderLayersTree() {
            ...
            html += `
                <div class="tree-item" style="margin-top: 20px;">
                    <div class="tree-node expandable expanded" onclick="toggleExpand(this)">
                        <span>📍 Coordination Layer (協調層)</span>
                    </div>
                </div>
                <div class="tree-children">
                    <div class="tree-item tree-indent">
        ...
        function renderOfficialMarket() {
            if (!officialCatalog) {
                document.getElementById('officialMarketList').innerHTML = '<p style="color: #999; text-align: center; padding: 40px;">載入官方目錄失敗</p>';
                return;
            }
        ...
        function renderOfficialList(items, filter = { category: 'all', status: 'all' }) {
            ...
            if (filteredItems.length === 0) {
                container.innerHTML = '<p style="color: #999; text-align: center; padding: 40px;">沒有符合條件的項目</p>';
                return;
            }
            ...
            return `
                <div class="official-card ${item.installed ? 'installed' : ''}">
                    ...
                    <div class="official-card-meta">
                        ...
                        ${item.auto_update ? '<span>🔄 自動更新</span>' : ''}
        ...
        async function installOfficial(id, type) {
            const confirmed = confirm(`確定要安裝 ${id} (${type === 'skill' ? 'Skill' : 'Team'}) 嗎？\n\n這將執行安裝腳本並可能需要幾秒鐘時間。`);
            if (!confirmed) return;
        ...
        async function uninstallOfficial(id, type) {
            const confirmed = confirm(`確定要移除 ${id} (${type === 'skill' ? 'Skill' : 'Team'}) 嗎？\n\n這將刪除已安裝的檔案。`);
            if (!confirmed) return;
        ...
        async function updateOfficial(id) {
            const confirmed = confirm(`確定要更新 ${id} 嗎？\n\n這將從 upstream 拉取最新版本。`);
            if (!confirmed) return;
        ...
        function toggleExpand(element) {
            ...
            // 找到下一個 tree-children 容器
            let sibling = element.parentElement.nextElementSibling;

            // 如果下一個元素是 tree-children，切換其顯示狀態
